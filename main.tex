...
Given a sampling scheme $\V = \{x_0, \dots, x_{n-1}\}$, we define $\sigma_i$ to be the patch of the surface of the sphere corresponding to $x_i$, $A_i$ its corresponding area, and $d_i$ the radius of the smallest ball in $\R^3$ containing $\sigma_i$ and centered at $x_i$.%\footnote{$d_i$ should be understood as the largest distance between the center $x_i$ and any point on the surface $sigma_i$.}
...
\end{theorem}
As a direct implication of this theorem, the rescaled operator $\hat L_n^{t_n} := \frac{|\mathbb S^2|}{4\pi t^2}L_n^{t_n}$ (and its discrete counterpart $ \mathbf{\hat L} = \frac{|\mathbb S^2|}{4\pi t^2} \mathbf L$) converges pointwise towards the Laplace-Beltrami operator ($\Delta_{\S^2}$) in the sense explained in \cite{belkin2007convergence}. This operator commutes with rotation and hence this convergence is a strong hint for equivariance, as formalized in the following proposition.
%%%%%%%%%%%%%%%%%%%%%%%  link between consistency and equivariance %%%%%%%%%%%%%%%%%%%%%%%%%
\begin{prop}\label{prop:equivariance}
Under the hypothesis of theorem \ref{theo:pointwise convergence for a regular sampling}, we have the following pointwise convergence for the rescaled graph Laplacian 
\begin{equation*}
