...
\newcommand{\mdeff}[1]{{\color[rgb]{.3,.8,.3}{#1}}}
...
TODO:
\begin{itemize}
	\item Unify vocabulary. Sampling, discretization, or pixelization of the sphere? Signal, maps, fields, functions. Irregular, uneven. Coarsening, down-sampling. Sample, vertex, node, pixel, point.
	\item check English vs American English
	\item Explain heuristic used to set the kernel width in v1. Add values to \figref{kernel_widths} for comparison. \nati{Not sure what you mean here.}
...
Not done:
\begin{itemize}
	\item Do the experiments on the icosahedral grid with the pooling as defined in the paper (i.e., not Jiang's specialty of taking the value of a pre-chosen sub-pixel). \nati{I guess we are not doing that.} \mdeff{I'm not sure what is the state of this, and Frédérick is not with us anymore. It's quite minor.}
...
In~\cite{perraudin2019deepspherecosmo}, the parameter $t$ was heuristically set to $0.5$ times the average squared distances over all connected pixels.
...
% We observe that with a small kernel size $t$, \cite{perraudin2019deepspherecosmo} would probably be more equivariant.
...
\begin{figure}
	...
	\begin{minipage}{0.35\linewidth}
		...
		\caption{Optimal kernel widths. For comparizon, we added the kernel used by~\cite{perraudin2019deepspherecosmo}}
