...
Given a sampling scheme $V=\{x_i\in\S^2, i=0, \dots, n\}$ of the sphere and a function $f : \S^2 \supset F_V \to \R$, the sampling operator $T_V: L^2(\S^2) \supset F\to \R^n,\  T_V(f) = \b{f}$ is defined as $\b{f}:\ f_i=f(x_i)$, where $F_V$ suitable subspace of $L^2(\S^2)$ such that $T_V$ is invertible, i.e., we can unambiguously reconstruct the function $f\in F_V$ from its sampled values $\b{f}$.
...
\begin{prop}\label{prop:1}
	For an equal area sampling $\{x_i\in\S^2\}_{i=0}^{n-1}: A_i=A_j \forall i,j$ of the sphere it is true that for all $f: \S^2 \rightarrow \R$ Lipschitz with respect to the Euclidean distance $\|\cdot\|$ with Lipschitz constant $\mathcal L_f$
	$$
...
\begin{proof}
	...
	Let us assume that the function $f:\R^3\rightarrow \R$ is Lipschitz with Lipschitz constant $\mathcal L_f$, we have
	...
	$$\left| \int_{\S^2}f({ x})\text{d}{\mu(x)} - \frac{1}{n}\sum_i f( x_i)\right| \leq \sum_i \left| \int_{\sigma_{i}}f({ x})\text{d}{\mu(x)} - \frac{1}{n}f( x_i)\right|\leq n  \mathcal L_fd^{(n)}\frac{1}{n} = \mathcal L_fd^{(n)}$$
...
\begin{prop}\label{prop:2}
	Given a sampling regular enough, i.e., for which we assume $A_i=A_j \ \forall i,j\text{ and }d^{(n)}\leq \frac{C}{\sqrt{n}}$, for a fixed $t>0$, a fixed Lipschitz function $f$ and a fixed point $y\in\S^2$ there exists a sequence $t_n = n^\beta, \beta<0$ such that
$$
...
\begin{align*}
	...
	&\leq  \max _{y\in \S^2} \left[\mathcal L_{\phi^t_y}d^{(n)} \right]\\
\end{align*}
...
$$||L_n^tf-L^tf||_\infty  \leq  \max _{y\in \S^2} \left[ \mathcal L_{\phi^t_y} \frac{C}{\sqrt{n}} \right]$$
...
So we can continue
$$\begin{aligned}
	\max _{y\in \S^2} \left[  \mathcal L_{\phi^t_y} \frac{C}{\sqrt{n}} \right]
