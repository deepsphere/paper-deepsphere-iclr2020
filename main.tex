...
\begin{itemize}
	...
	\item \citet{Mudigonda} used Integrated Water Vapor (IWV) only. That most probably is the same as our TMQ, which is most probably the most informative feature. TODO: run ours with TMQ alone.
\end{itemize}
}
...
A special case is the equiangular sampling scheme where a sampling theorem holds, and thus a closed form of $T_\V^{-1}$ is known. % \citep{driscoll1994Fouriersphere}
...
Following their setting, we will work with the extended graph Laplacian operator as the linear operator $L_n^t: L^{2}(\mathcal{M}) \rightarrow L^{2}(\mathcal{M})$ such that
	...
	\label{eq:Heat Kernel Graph Laplacian operator}
	 L_n^t f(y) := \frac{1}{n}\sum_{i=0}^{n-1} e^{ -\frac{\|x_i-y\|^2}{4t}} \left(f(y)-f(x_i)\right).
	\end{equation}
...
As the radius of the kernel $t$ will be adapted to the number of sample, we also rescale the graph Laplacian operator as
    % \begin{equation}
	% \label{eq:rescaled Heat Kernel Graph Laplacian operator}
...
Given a sampling scheme $\V = \{x_0, \dots, x_{n-1}\}$, we define $\sigma_i$ to be the patch of the surface of the sphere corresponding to $x_i$, $A_i$ its corresponding area, and $d_i$ as the largest distance between the center $x_i$ and any point on the surface $\sigma_i$.
 Define $d^{(n)} := \max_{i=0, \dots, n}d_i$ and $A^{(n)}=\max_{i=0, \dots, n}A_i$.
...
\end{equation}
\begin{theorem}
	For a sampling $\V = \{x_i\in\S^2\}_{i=0}^{n-1}$ of the sphere that is equi-area and such that $d^{(n)} \leq \frac{C}{n^\alpha}, \ \alpha\in (0,\linefrac{1}{2}]$, for all $f: \S^2 \rightarrow \R$ Lipschitz with respect to the Euclidean distance in $\R^3$, for all $y\in\S^2$, there exists a sequence $t_n = n^\beta$, $\beta\in\mathbb R$ such that 
...
For 8 neighbors, selecting the optimal value of $t$ already leads to an improvement compared to \citet{perraudin2019deepspherecosmo}, whose graph also was constructed to have at maximum 8 neighbors. 
...
\begin{itemize}
  \item As a reminder of \eqref{eq:Heat Kernel Graph Laplacian operator}, the extended graph Laplacian operator is a linear operator $L_n^t: L^{2}(\mathcal{M}) \rightarrow L^{2}(\mathcal{M})$  is defined as
	\begin{equation}
	\label{eq:Heat Kernel Graph Laplacian operator}
