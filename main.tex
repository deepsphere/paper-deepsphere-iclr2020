...
\end{equation}
This operator extends the graph Laplacian operator previously defined with the weighting scheme \eqnref{weights:belkin} to each point of the manifold, i.e., the sphere.
...
\end{theorem}
As a direct implication of this theorem, the discrete counterpart of $\mathbf{\hat L}$ \mdeff{(that's already discrete no?)} converges pointwise towards the Laplace-Beltrami operator $\Delta_{\S^2}$ in the sense explained in \citep{belkin2007convergence}.
This operator commutes with rotation and hence Theorem \ref{theo:pointwise convergence for a regular sampling} is a strong step towards equivariance. We formalize it in the following theorem.
\mdeff{This transition should be done better.}
