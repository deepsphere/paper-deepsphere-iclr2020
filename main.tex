...
We extend the work of Belkin and Nyiogi to a sufficiently regular, deterministic sampling scheme. For this purpose we need now to introduce some definitions. Following their setting, we define the extended graph Laplacian operator as the linear operator $L_n^t: L^{2}(\mathcal{M}) \rightarrow L^{2}(\mathcal{M})$ such that
	\begin{equation}
	\label{eq:Heat Kernel Graph Laplacian operator}
	 L_n^t f(y) := \frac{1}{n}\sum_{i=0}^{n-1} e^{ -\frac{\|x_i-y\|^2}{4t}} \left(f(y)-f(x_i)\right).
	\end{equation}
and the rescaled graph Laplacian operator
    \begin{equation}
	\label{eq:rescaled Heat Kernel Graph Laplacian operator}
	 \hat L_n^t := \frac{|\mathbb S^2|}{4\pi t^2}L_n^t
	\end{equation}
Note that we have the following relations $\b{L}_n^t \b{f} = T_\V L_n^t f$, $\b{\hat L} = \frac{|\mathbb S^2|}{4\pi t^2}\b L_n^t$. Given a sampling scheme $\V = \{x_0, \dots, x_{n-1}\}$, we define $\sigma_i$ to be the patch of the surface of the sphere corresponding to $x_i$, $A_i$ its corresponding area, and $d_i$ as the largest distance between the center $x_i$ and any point on the surface $\sigma_i$.
...
% 	the rescaled Heat Kernel Graph Laplacian $\frac{|\S^2|}{4\pi t_n^2}\b {L}^t_n$ converges pointwise to the Laplace Beltrami operator on the sphere $\Delta{\S^2}$  
	\begin{equation*}
		\lim_{n\to\infty}\hat L_n^{t_n}f(y) = \triangle_{\S^2}f(y).
	\end{equation*} 
	\label{theo:pointwise convergence for a regular sampling}
\end{theorem}
As a direct implication of this theorem, the discrete counterpart $ \mathbf{\hat L}$ converges pointwise towards the Laplace-Beltrami operator ($\Delta_{\S^2}$) in the sense explained in \cite{belkin2007convergence}. This operator commutes with rotation and hence theorem \ref{theo:pointwise convergence for a regular sampling} is a strong hint for equivariance, as formalized in the following proposition.
...
\begin{itemize}
    \item The extended graph Laplacian operator \eqref{eq:Heat Kernel Graph Laplacian operator}
