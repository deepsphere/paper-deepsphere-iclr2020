...
\begin{abstract}
Selecting the right convolution for a spherical neural network is a delicate tradeoff between efficiency and  exactitude with respect of equivariance. DeepSphere, as a graph based method strikes as a controllable balance between these two desiderata. 
This contribution (i) studies empirically and theoretically how the construction of its underlying graph affects equivariance, and (ii) evaluates its performance on a large set of relevant problems.
...
\begin{theorem}
	For a sampling $\V = \{x_i\in\S^2\}_{i=0}^{n-1}$ of the sphere that is equi-area and such that $d^{(n)} \leq \frac{C}{\alpha}, \ \alpha\in (0,\linefrac{1}{2}]$, for all $f: \S^2 \rightarrow \R$ Lipschitz with respect to the Euclidean distance in $\R^3$, for all $y\in\S^2$, there exists a sequence $t_n = n^\beta$ such that 
