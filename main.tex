...
Examples of (ii) is the observation of the universe, LIDAR scans, and the digitalization of 3D objects. % through which apparatus?
See \figref{examples}.
...
Neural networks (NNs) have proved to be great tools to solve inference problems. % when a huge amount of data is available.
...
For most samplings, it is not known if $F_V$ exist and hence if $T_V$ is invertible. A special case is the \textit{equiangular sampling} scheme where a sampling theorem holds, and thus we have a closed form of $T_V^{-1}$ is known. % \cite{driscoll1994Fouriersphere}
...
We extend the work Belkin and Nyiogi to a fixed sampling. 
...
\begin{theorem}
	...
	$$ \lim_{n\to\infty} \frac{|\mathbb{S}^2|}{4\pi t_n} \b{L}_n^{t_n} T_{V_n} f(y) =  \Delta_{\mathbb{S}^2}f(y).$$
	\label{theo:pointwise convergence for a regular sampling}
\end{theorem}
This theorem show that the $\b{L}$ converges pointwisely toward an operator ($\Delta_{\mathbb S^2}$) commuting with rotation and hence is a strong hints for equivariance.
...
Unfortunately, the graph laplacian does \emph{not} converges uniformly in general \cite{belkin2007convergence}. This can probably be overcomed by considering a specific class of functions, such as bandlimited function.
...
From 20\% error on V1, we got to less than 2\% error on v2. More neighbors, more precision, plus a better kernel width obtained "minimizing" the equivariance error. How to explain how we found the kernel width? Because minimizing the kernel width would lead to a completely disconnected graph. What we did was finding a heuristic law (fix neighbors, set the kernel width by imposing the minimum weight to be equal to $0.1$) and then optimizing
\begin{figure}
	\centering
