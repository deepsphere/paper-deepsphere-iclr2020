...
Working specifically with the equiangular sampling, they consider the set $\mathcal{G}$ of all the possible graphs where each node is connected only to four of its nearest neighbours (North, Sud, West, East).
...
\begin{table}
    ...
    \begin{tabular}{l c c c}
		...
		8 neighbors, $\hat{t} = ??$, FCN variant, \citep{perraudin2019deepspherecosmo} & 83.8 & \todo{??} \\
		8 neighbors, optimal kernel width $\hat{t} = ??$ & 87.1 & \todo{??} \\
		20 neighbors, optimal kernel width $\hat{t} = ??$ & 91.25 & \todo{??} \\
