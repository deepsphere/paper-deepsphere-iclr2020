...
As the sphere is a compact smooth manifold embedded in $\mathbb{R}^3$, we can reuse the second part or their proofs where \eqref{eq:Functional approximation to the Laplace-Beltrami operator} $\rightarrow$ \eqref{eq:laplace-beltrami}.
Thus, our strategy to prove Theorem \ref{theo:pointwise convergence for a regular sampling} is to a) show that 
...
% it is known that  (\cite{belkin2005towards}) to prove the convergence of $(\b {L}_n^t \b f) _i$ to $\Delta_{\mathbb S^2}f(x_i)$ it is sufficient to prove the convergence of its continuous counterpart $L_n^tf(x)$ to $\Delta_{\mathbb S^2}f(x)$
