...
As a lack of equivariance can penalize performance (\secref{exp:cosmo}) and expensive convolutions prohibit their application to some real-world problems, methods standing between those two extremes are desired.
...
In this work, we argue that graph-based spherical CNNs strike an interesting balance, with a controllable tradeoff between cost and performance (linked to equivariance).
...
We calculated numerically the equivariance error \eqref{eq:mean equivariance error} for monochromatic signals, for different parameter sets of DeepSphere and for the graph of Khasanova and Frossard. We approximated equation \eqref{eq:mean equivariance error} by calculating the empirical average of the equivariance errors of $N$ random monochromatic signals on $M$ random rotations for each signal. Results are in figure \ref{fig:equivariance_error}.
...
\begin{figure}
	\begin{minipage}{0.65\linewidth}
		...
		\caption{Equivariance error. The number of pixels is $4*s^2$ equiangular = 

Npix for healpix = 12*nside**2}
