...
We calculated numerically the equivariance error \eqref{eq:mean equivariance error} for different parameter sets of DeepSphere and for the graph of Khasanova and Frossard. We approximated equation \eqref{eq:mean equivariance error} by calculating the empirical average of the equivariance errors of $N$ random monochromatic signals on $M$ random rotations for each signal. Results are in figure \ref{fig:equivariance_error}.
...
Following \citet{perraudin2019deepspherecosmo}, the NN is made of $6$ graph convolutional layers, each followed by a max pooling layer which down-samples by $4$.
A GAP and softmax follow.
The polynomials are all of order $K=5$ and the number of channels per layer is $16, 32, 64, 64, 64, 2$, respectively.
