...
We calculated numerically the equivariance error \eqref{eq:mean equivariance error} by doing an empirical average on $N$ random monochromatic signals, and $M$ random rotations for each signal, for different parameter sets of DeepSphere and for the graph of Khasanova, Frossard. Results are in figure \ref{fig:equivariance_error}.
The frequencies of the monochromatic signals, on the x axis in figure \ref{fig:equivariance_error}, were chosen in the range $(0,\  3N_{side}-1)$ to allow for an almost perfect synthesis of the sampled signals, and thus an almost perfect implementation (up to numerical errors) of the operator $\b{R}_\V$ in equation \eqref{eq:mean equivariance error} \citep{gorski1999healpixprimer}. In this way all the error measured is due to imperfections in the empirical approximation of the Laplace-Beltrami operator.

% 3.1) explain what the figures tell us
We observe that by increasing the number of neighbors of the graph from 8 to 40, we are able to decrease the equivariance error by more than one order of magnitude. The filtering operation with the graph Laplacian of Khasanova, Frossard is by construction perfectly equivariant to rotations along the vertical axis, and is designed to minimize the filtering error of vertical displacements of the same image. While these characteristics are perfect for analysing omnidirectional images that are naturally aligned on the north-south axis, it doesn't perform well on random rotations. 
\begin{figure}
	\centering
	\includegraphics[width=0.5\linewidth]{all_in_one.pdf}
	\caption{Equivariance Error}
...
\begin{table}
    ...
    \begin{tabular}{l c c c c}
		\toprule
		& accuracy & conv. equiv. error & inference \\ % & training time & NN inv. error
		\midrule
		2D CNN baseline \citep{perraudin2019deepspherecosmo} & \todo{??} &  \\ % &  & \todo{can we?}
		8 neighbors, CNN variant, \citep{perraudin2019deepspherecosmo} & \todo{??} & \todo{??} &  \\
		8 neighbors, FCN variant, \citep{perraudin2019deepspherecosmo} & 83.8 & \todo{??} & 187\,ms \\ % & 1\,h\,45\,m
		8 neighbors, optimal kernel width & 87.1 & \todo{??} & 185\,ms \\ % & 1\,h\,45\,m
		20 neighbors, optimal kernel width & 91.3 & \todo{??} & 250\,ms \\ % & 2\,h\,30\,m
		40 neighbors, optimal kernel width & 92.5 & \todo{??} & 363\,ms\\ % & 3\,h\,45\,m
		\bottomrule
    \end{tabular}
	\caption{
		...
		\todo{add speed to show trade-off}
	}
    \label{tab:cosmo}
\end{table}
